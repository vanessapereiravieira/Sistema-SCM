...
if (USUARIO_ROLE !== 'admin') {
    window.location.href = '../../pages/perfil-user/minhas_manutencoes_user.html';
}
...
async function carregarManutencoes() {
    try {
        const resposta = await fetch(`${API_URL}/minhas-manutencoes?usuario_id=${USUARIO_ID}&role=${USUARIO_ROLE}`);
        const dados = await resposta.json();
        
        console.log("DADOS CRUS:", dados);
        
        // Forçar ser array se não for
        if (!Array.isArray(dados)) {
            console.error("Back-end não retornou array!");
            manutencoes = [];
        } else {
            manutencoes = dados;
        }
        
        // Forçar campo nome_sala se não existir
        manutencoes = manutencoes.map(m => ({
            ...m,
            nome_sala: m.nome_sala || m.nomeSala || "Não informado",
            criado_em: m.criado_em || m.created_at || new Date().toISOString()
        }));
        ...
        if (manutencoes.length === 0) {
            alert("Nenhuma manutenção encontrada. Cadastre uma primeiro!");
        }
    } catch (erro) {
        console.error("FALHA TOTAL:", erro);
    ...
    try {
        ...
        const resposta = await fetch(
            `${API_URL}/nova-manutencao/${id}`,
            {
                ...
                body: JSON.stringify({
                    usuario_id: manutencaoRemovida.usuario_id,
                    role: USUARIO_ROLE
                })
            }
        );
...
function editarManutencao(id) {
    // Admin pode editar qualquer manutenção
    window.location.href = `nova_manutencao_admin.html?editar=${id}&usuario_id=${USUARIO_ID}&role=${USUARIO_ROLE}`;
}
