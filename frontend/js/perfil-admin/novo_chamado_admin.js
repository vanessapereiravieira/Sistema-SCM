...
// Verificar se é admin (mudança aqui)
if (USUARIO_ROLE !== 'admin') {
    window.location.href = '../../pages/perfil-user/novo_chamado_user.html';
}
...
async function enviarChamado(evento) {
    ...
    const dados = {
        ...
        usuario_id: USUARIO_ID  // USA O ID DO PRÓPRIO ADMIN
    };
    ...
    try {
        ...
        if (resposta.ok) {
            ...
            limparFormulario();
            // Redireciona para página de chamados do admin
            setTimeout(() => window.location.href = 'meus_chamados_admin.html', 1500);
        } else {
...
async function carregarChamadoParaEdicao() {
    ...
    try {
        // Para admin, usa chamados-encaminhados com role=admin para ver todos
        const resposta = await fetch(
            `${API_URL}/chamados-encaminhados?usuario_id=${USUARIO_ID}&role=admin`
        );
